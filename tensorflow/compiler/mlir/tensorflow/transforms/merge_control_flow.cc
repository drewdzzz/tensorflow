...
// Check if the `potential_last` IfRegion can be added to the segment of
// IfRegion start with `current` IfRegion
bool can_add_to_if_segment(
  ...
  TF::IfRegionOp second_if_op = *potential_last;

  for (auto iter = std::prev(potential_last); std::next(iter) != current;
       iter--) {
    ...
    downstream_if_ops = get_middle_if_ops(*(std::next(iter)));
...
IfOpIterConst FindSubgroupLastIfOpIter(
    ...
    const llvm::SmallVector<mlir::TF::IfRegionOp, 8>& if_ops,
    const std::unique_ptr<TF::SideEffectAnalysis>& side_effect_analysis) {
  if (std::next(current) == if_ops.end()) {
    return current;
  }

  IfOpIterConst res = current;
  while (can_add_to_if_segment(current, res, if_ops, side_effect_analysis)) {
    res = std::next(res);
  }
  return std::prev(res);
}
...
// `sub_if_group` refers to a segment of IfRegions, the ops are in the
// sequential order.
...
// Checks whether a return indice should be kept for `current_if_op` by checking
