...
// it.
struct ShapeIndexView : public absl::Span<const int64_t> {
  using Span::Span;
  ShapeIndexView(absl::Span<const int64_t> indices) : Span(indices) {}
  ShapeIndexView(const ShapeIndex& shape_index, int64_t offset = 0)
      : Span(shape_index.data() + offset, shape_index.size() - offset) {
    CHECK_LE(offset, shape_index.size());
  }

  ShapeIndexView ConsumeFront() const { return subspan(1); }
  ShapeIndexView ConsumeBack() const { return first(size() - 1); }
