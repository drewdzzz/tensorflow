...
class PjRtEvent {
 public:
  ...
  explicit PjRtEvent(T t)
      : event_(tfrt::MakeAvailableAsyncValueRef<T>(t)),
        on_block_start_([]() { return PjRtEventHelpers::ProfilingKeys(); }),
        on_block_end_([](PjRtEventHelpers::ProfilingKeys) {}),
        host_ctx_(nullptr) {}
  ...
  explicit PjRtEvent(
      ...
      PjRtEventHelpers::OnBlockStartFn on_block_start =
          []() { return PjRtEventHelpers::ProfilingKeys(); },
      PjRtEventHelpers::OnBlockEndFn on_block_end =
          [](PjRtEventHelpers::ProfilingKeys) {})
  ...
  // the wrapped PjrtEvent<T>::Event class and block without using HostContext.
  //
  // on_block_start is called before BlockHostUntilReady starts to block.
  // on_block_end is called after BlockHostUntilReady finishes blocking.
  explicit PjRtEvent(
  ...
  T BlockHostUntilReady() {
    if (!event_.IsAvailable()) {
      ...
      if (host_ctx_) {
        ...
        tfrt::Await({event_.GetAsyncValue()});
      }
      on_block_end_(keys);
    }
 ...
 private:
  ...
  PjRtEventHelpers::OnBlockStartFn on_block_start_;
  // Function that is called after a thread finishes blocking on the event.
  PjRtEventHelpers::OnBlockEndFn on_block_end_;
